...
def generate_response_stream(model, messages, session_id: str):
    ...
    try:
        ...
        import re
        unwanted_pattern = r'(?:Важно|Обратите внимание|Примечание):?.*?(?:носит общий характер|не является юридической консультацией|необходимо обратиться к квалифицированному юристу|обратитесь в ЦОН|правоохранительные органы|другому юристу).*?(?:\.|\n)'
